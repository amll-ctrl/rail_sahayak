...
exports.notifyPassengerOfRequestStatus = onDocumentUpdated('requests/{requestId}', async (event) => {
  ...
  if (passengerId) {
    ...
    const token = passengerDoc.exists ? passengerDoc.data()?.fcmToken : null;
    if (token) {
  ...
  if (status === 'At Station') {
    const station = String(after.boardingStation || 'your boarding station');
    const location = String(after.currentLocation || 'Location not specified');
    const platform = String(after.platform || 'Platform not specified');
    const staffSnapshot = await db.collection('users').where('role', '==', 'staff').get();
    const staffTokens = staffSnapshot.docs.map((doc) => doc.data()?.fcmToken).filter(Boolean);
...
exports.markPassengerAtStation = onCall(async (request) => {
  ...
  const platform = String(request.data?.platform || '').trim().slice(0, 40);
  const location = String(request.data?.location || '').trim().slice(0, 120);
  const update = {
    status: 'At Station',
...
exports.updateAssistanceStatus = onCall(async (request) => {
  ...
  const ref = db.collection('requests').doc(requestId);
  const staffName = String(userDoc.data()?.name || 'Railway Staff');
  let previousStatus = '';
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const update = { status: nextStatus, staffId: uid, staffName };
    const nowField = { Assigned: 'acknowledgedAt', Located: 'locatedAt', Boarding: 'boardingAt', Boarded: 'boardedAt', Completed: 'completedAt' }[nextStatus];
    if (nowField) update[nowField] = FieldValue.serverTimestamp();
    transaction.update(ref, update);
  });

  await writeAudit(requestId, nextStatus, { uid, role: 'staff' });
  logger.info('Assistance workflow transition', { requestId, previousStatus, nextStatus, staffId: uid });
  return { status: nextStatus };
});

exports.escalateAssistanceRequest = onCall(async (request) => {
  if (!request.auth) throw new HttpsError('unauthenticated', 'Please sign in to escalate assistance.');
  const uid = request.auth.uid;
  ...
  await ref.update({ status: 'Escalated', escalatedAt: FieldValue.serverTimestamp(), escalatedBy: uid });
  await writeAudit(requestId, 'Escalated', { uid, role: 'staff' });
...
exports.getTrainInfo = onCall({ secrets: [railRadarApiKey] }, async (request) => {
  ...
  try {
    const response = await fetch(`${baseUrl}/legacy/trains/${trainNumber}?dataType=full`, { headers: { Authorization: `Bearer ${apiKey}`, Accept: 'application/json' } });
    if (!response.ok) {
